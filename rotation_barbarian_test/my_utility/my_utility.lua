...
local function elites_in_shouts()
    local units = {}
    local player_position = get_player_position()
    local units_in_range = utility.get_units_inside_circle_list(player_position, 8.0)
    for _, unit in ipairs(units_in_range) do
        if unit:is_elite() or unit:is_champion() or unit:is_boss() then
...
local plugin_label = "Base_Barbarian_Plugin_"
