...
local function is_valid_area_spell_percentage(area_table, entity_list, min_percentage)
    -- Check if entity_list is nil or empty
    if not entity_list or #entity_list == 0 then
        return false;
    end
    
    -- Ensure area_table is valid
    if not area_table.is_valid then
        return false;
    end
    ...
    -- Return true if percentage is greater than or equal to min_percentage
    if percentage >= min_percentage then
        return true;
    end

    return false; -- Always have a return false at the end in case conditions aren't met
end
...
local function is_valid_area_spell_aio(area_table, min_hits, entity_list, min_percentage)
    ...
    -- Check percentage-based conditions
    if is_valid_area_spell_percentage(area_table, entity_list, min_percentage) then
        return true;
    end
    
    return false; -- Return false if neither of the above conditions is met
end
